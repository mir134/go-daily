...
function CustomTooltip({
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-3 border text-sm space-y-1">
      <p className="text-gray-500">{d.date}</p>
      {d.morningLabel != null && (
        <p className="font-semibold" style={{ color: '#10b981' }}>🌅 早上: {d.morningLabel}</p>
      )}
      {d.eveningLabel != null && (
        <p className="font-semibold" style={{ color: '#f59e0b' }}>🌙 晚上: {d.eveningLabel}</p>
      )}
    </div>
  )
}
...
export default function TrendsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      {/* 风险预警区 */}
      <div>
        <h2 className="text-xl font-bold mb-4">风险预警区</h2>
        {alerts.length === 0 ? (
          <p className="text-green-600 font-medium">目前没有发现风险趋势 ✓</p>
        ) : (
          <div className="space-y-3">
            {alerts.map((alert, i) => (
              <Card key={i} title={`⚠ ${alert.type}`}>
